  ...
  return (
    <div className="m-4 flex flex-col items-center">
      ...
      <div className="flex flex-col w-auto">
        ...
        <Slider style={{width: "auto"}} className="mx-4 my-2" name="Length" min={charsets.length} valueLabelDisplay="on" value={length} onChange={(_, val) => setLength(val as number)} />
        <CheckBox name="Include uppercase letters (A-Z)" checked={includeUppercase} onChange={() => setIncludeUppercase(!includeUppercase)} />
        <CheckBox name="Include numbers (0-9)" checked={includeNumbers} onChange={() => setIncludeNumbers(!includeNumbers)} />
        <CheckBox name="Include symbols (!@#...)" checked={includeSymbols} onChange={() => setIncludeSymbols(!includeSymbols)} />
      </div>
    </div>
  )
}
